...
| Mining anti-exploit | Place an ore, reconnect, push and retract it with a piston, then break it; flow water/lava from a bucket across a configured ore/log and break the destination; test explosion/fake-player path where enabled | Placement/piston/fluid/break screenshots and server log proving zero or expected XP after movement; include bucket source and destination coordinates |
