...
| Mining | Break configured stone/ores with the correct tool; compare Silk Touch/Fortune and ability states; verify Double Drops/Mother Lode on allowlisted Diamond Ore versus a non-allowlisted XP block such as Reinforced Deepslate | Before/after inventory screenshot, XP text, exact block IDs, `mining-drops.properties`, server log and coordinates |
...
| Woodcutting | Break vanilla and configured modded logs; test Tree Feller, Leaf Blower, durability and placed logs; compare normal Tree Feller logs against the configured bonus-drop allowlist | Screenshots of normal versus Tree Feller drops/tool durability/XP, block IDs and `woodcutting-drops.properties`, plus server log |
