...
| Combat | Test Swords, Axes, Unarmed, Archery, Crossbows, Tridents, Maces and Spears in PvE and PvP where allowed; include a tameable/horse target to verify `combat.tamed_mob_xp_multiplier`; test Unarmed Disarm with `combat.unarmed.disarm_anti_theft=0` and `=1`, including owner and another player attempting pickup; for Serrated Strikes/Skull Splitter place a normal mob, another player's pet, a spectator and a player nearby | Before/after health/armor/inventory/XP screenshots, target ownership, owner UUID, pickup player identity, chat feedback and server log proving only eligible secondary targets were affected |
