...
* Mining e Woodcutting usam tabelas próprias por registry ID derivadas do `experience.yml` fixado; Woodcutting também usa `skills/woodcutting-drops.properties` para a allowlist de bonus drops equivalente ao mcMMO. A resolução não fica mais hardcoded nos adapters. Provenance, actor validation e blocos colocados continuam no pipeline comum.
