...
| Mining | blocks | drops/Super Breaker/Blast Mining/Bigger Bombs | pickaxe/drop path; mcMMO mining bonus-drop allowlist, configurable Silk Touch, rank caps, remote blast radius gate and fail-closed TNT provenance | pickaxe/drop path; mcMMO mining bonus-drop allowlist, configurable Silk Touch, rank caps, remote blast radius gate and fail-closed TNT provenance |
...
| Woodcutting | logs | drops/Tree Feller/Leaf Blower | log/drop path; configured Tree Feller limit, reduced XP count including the initial log, per-log Tree Feller bonus drops and placed-block guard | log/drop path; configured Tree Feller limit, reduced XP count including the initial log, per-log Tree Feller bonus drops and placed-block guard |
...
Verification on 2026-08-11: the API/common/persistence suite has 119 tests, `./gradlew clean build`, Fabric remapJar, NeoForge jar, latest Fabric/NeoForge compilation, final-JAR shared-class inspection, and `git diff --check` pass. An offline comparison also matched all 466 numeric `Experience_Values` entries from the fixed baseline, including 129 Mining and 50 Woodcutting values. No Minecraft client/server task was launched in this validation cycle by explicit project rule. Earlier boot/session evidence remains historical only and is not being repeated automatically.
...
| Wrong XP | PARTIAL | Fixed reference action tables plus standard linear and configurable exponential curves are tested; combat XP now uses effective damage times base XP with the reference 100-damage ceiling, tamed-target multiplier, and spawner/egg/nether-portal/breeding origin multipliers, while Tree Feller reduced XP and native Understanding the Art cooking XP mutation are wired on both loaders, with player smoke still pending. |
...
| Anti-exploit | PARTIAL | Placement provenance, actor checks, fail-closed Blast Mining provenance, piston and bucket-fluid transfer of placed-block provenance, explosion marker cleanup, Green Thumb replant provenance, bounded chain effects, persistent Call of the Wild summon tags that block combat XP, tamed/spawner/egg/nether-portal/bred combat-origin tags, expiring summon ownership, optional Unarmed AntiTheft protection, fishing rapid/stationary guard and optional diminished returns are active; explosion-origin attribution, WorldEdit/quarries and other modded provenance sources remain. |
