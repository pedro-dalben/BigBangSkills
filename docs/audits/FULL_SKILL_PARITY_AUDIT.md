...
| Wrong XP | PARTIAL | Fixed reference action tables plus standard linear and configurable exponential curves are tested; combat XP now uses effective damage times base XP with the reference 100-damage ceiling and the default tamed-target multiplier of zero, Tree Feller reduced XP and native Understanding the Art cooking XP mutation are wired on both loaders, with player smoke still pending. Spawn-reason/egg/bred/portal multipliers remain explicitly unimplemented. |
