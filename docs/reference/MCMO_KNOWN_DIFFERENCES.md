...
| mcMMO integrations preserve placed-block provenance through piston and fluid movement | Vanilla piston/fluid movement has no shared cross-loader event contract | Both loaders hook `PistonBaseBlock.moveBlocks` and transfer tracked provenance for pushed/retracted blocks; bucket fluids are marked and `LiquidBlock` flow propagates/clears the mark; external machines and non-tracked explosions remain integration gaps |
