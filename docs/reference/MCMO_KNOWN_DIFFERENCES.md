...
| mcMMO receives a Bukkit `NETHER_PORTAL` creature spawn reason | Fabric/NeoForge do not expose one portable persistent portal-origin flag at the combat hook | Spawner, spawn-egg and breeding origins are tagged and configurable; nether-portal origin remains a documented gap rather than an inferred heuristic |
