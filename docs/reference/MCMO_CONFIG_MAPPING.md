...
| Skill detail metadata | common `SkillMessageFormatter` | `/skills <skill>` dynamically shows XP curve parameters, passive/active modes and ranks, activation syntax, cooldowns, enabled/PvP/PvE/ability restrictions and per-skill XP multiplier |
...
| `Bonus_Drops.*` | configured drop modifiers | Mining/Woodcutting/Herbalism Double Drops and Verdant Bounty are level/formula driven; Mining and Woodcutting defaults/overrides live in `skills/mining-drops.properties` and `skills/woodcutting-drops.properties`; Blast Mining bonus drops are configurable with rank multiplier/chance |
...
| `Green_Thumb_Replanting_Crops.<crop>` | `herbalism.replant_<crop>` | defaults enabled for wheat, carrots, potatoes, beetroots, nether wart, cocoa and sweet berry bush; loaders consume the per-crop gate |
