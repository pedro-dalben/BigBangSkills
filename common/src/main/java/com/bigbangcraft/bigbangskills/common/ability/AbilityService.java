...
public final class AbilityService {
    ...
    public boolean activate(UUID playerId, AbilityDefinition ability, int level, Instant now) {
        if (level < ability.unlockLevel() || !cooldowns.consume(playerId, ability.id(), ability.cooldown())) return false;
        if (ability.type() == AbilityType.ACTIVE) active.computeIfAbsent(playerId, ignored -> new ConcurrentHashMap<>()).put(ability.id(), now.plus(ability.duration()));
        return true;
    }
